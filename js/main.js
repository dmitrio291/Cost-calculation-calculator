document.addEventListener('DOMContentLoaded', () => {
    ...
    function calculate() {
        ...
        for (const radio of radioType) {
            if (radio.checked) {
                totalPrice *= parseFloat(radio.value); // 300 000 * 1.2
            }
        }

        for (const radio of radioBuilding) {
            if (radio.checked) {
                totalPrice *= parseFloat(radio.value); // 360 000 * 1.1 = 390 000
            }
        }

        for (const radio of radioRooms) {
            if (radio.checked) {
                totalPrice *= parseFloat(radio.value); // 390 000 * 0.8 = 350 000
            }
        }

        if (ceilings.checked) {
            totalPrice += parseFloat(ceilings.value) * parseInt(squareInput.value);
        }
